...
import { GetProductsUseCase } from '@/usa-case/product/get-all-products-use-case'
...
const productController = new ProductController(productCreateUseCase, productUploadImageUseCase, getProductsUseCase, getProductByIdUseCase)
...
export function productsRoutes(app: FastifyInstance) {
    ...
    app.get('/:id', async (request: FastifyRequest, reply: FastifyReply) => productController.getProductById(request, reply))
}
